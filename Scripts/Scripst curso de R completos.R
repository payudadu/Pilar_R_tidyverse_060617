...
#Hacer strings. Lo primero, función rep (repeat) y poner dos parámetros, 
#para hacer un string: lo que quieres repetir y cuántas veces repetirlo

rep("This is an example", times=3)

#Otra forma de escribirlo, que puede ser más sencilla de leer para el futuro
"This is an example" %>% rep(times=3)

#Restrict the gapminder dataframe. Está creando un objeto y con select se escogen
#los datos a las columnas que nos interesan

year_country_gdp <- select(gapminder, year, country, gdpPercap)
head(year_country_gdp)                          

#Usar el pipe command %>%: sirve para repetir algo en las siguientes lineas

year_country_gdp <- gapminder %>% 
  select(year, country, gdpPercap)
head(year_country_gdp)  

#Filtrar gapminder según el año elegido, 2002, usa el pipe en ggplot funtion
#y señala el x e y. 
...
#Restringir aún más los datos, ahora a europa

year_country_gdp_euro <- gapminder %>%
  filter(continent=="Europe") %>% 
  select(year, country, gdpPercap)

#Challenge filter data for Norway
...
#functin summarize, que se aplica a cada grupo del dataframe
#Da una tabla más pequeña que es un resumen de los datos usando las medias de cada grupo

gapminder %>%
  group_by(continent) %>%
  summarise(mean_gdpPercap=mean(gdpPercap))

#Si no lo separas por países o por algo hace la media de todos los datos y da un 
#único dato
gapminder %>%
    summarise(mean_gdpPercap=mean(gdpPercap))

#Hacer un plot con los datos que hemos estado usando, puntos, usando las 
#medias de cada continente y un diagrama de puntos
...
#challenge: calculate the average life expectancy per country in Asia
